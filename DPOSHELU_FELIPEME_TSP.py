...
#import matplotlib.animation import FuncAnimation #http://matplotlib.org/api/animation_api.html#module-matplotlib.animation
...
if (len(sys.argv) == 9):
	...
	solveGA             = int(sys.argv[6])
	IPcutoffTime		= float(sys.argv[7])	# -1 --> No time limit, o.w., max number of seconds for Gurobi to run
	turnByTurn			= int(sys.argv[8])		# 1 --> Use MapQuest for detailed routes.  0 --> Just draw straight lines connecting nodes.
...
def solve_tsp_ga(objectiveType, distance_df, time_df,node):
	...
	MaxGenerations = 400	# Maximun number of generations 400
	PopSize = 800 			# Population size 800
	MutationRate = 0.01		# Mutation probability
	CrossOverRate = 0.75	# Crossover probability
	...
	for i in range(MaxGenerations):
		...
		BESTROUTE.append(XBest)
		# Print and Plot the stats (curr population)
		#print 'PopAverage=', PopAverage
		#ax2.plot(i,1.0/ZBestIndiv,'bo')
		MEANobj.append((1.0/ZBestIndiv))
		MINobj.append(PopAverage)
		#ax1.plot(i,PopAverage,'ko')
	...
	#animation (notlive)
	#http://stackoverflow.com/questions/33275189/how-can-i-make-points-of-a-python-plot-appear-over-time
	ITER = len(MEANobj)
	LOC = len(BESTROUTE[1])
	Iter = range(ITER)
	Loc = range(LOC)
	...
	ax1.set_xlim(0,ITER)
	ax1.set_ylim(50,(max(MEANobj)+20))
	graph, = ax1.plot([], [], 'o', markersize = 1)

	ax2.set_title('Route History')
	ax2.set_xlabel('X')
	ax2.set_ylabel('Y')
	ax2.set_ylim(42.825,43.025)
	ax2.set_xlim(-78.9,-78.7)
	graph1, = ax2.plot([], [], '-o', markersize = 4)

	def animate(j):
		graph.set_data(x[:j+1], y1[:j+1])

		Rou = BESTROUTE[j]
		x2 = list(Rou)
		y2 = list(Rou)

		#print 'Length of Rou is:', len(Rou)
		#print 'Length of Iter is:', len(Iter)
		#print 'Iter is:', Iter

		for k in Loc:
			COO = coordList[BESTROUTE[j][k]]
			#print 'COORDINATE IS:', COO
			#print 'COO is:', COO[1]
			x2[k] = COO[0]
			y2[k] = COO[1]
		...
		#x = [[0, 2, 3, 5], [2, 3, 5,
		#y = [[2, 3, 5, 1], [0, 2, 3, 5]]
		#graph.set_data(x[j], y[j])
		graph1.set_data(y2, x2)


		return graph, graph1

	ani =FuncAnimation(fig, animate, frames=ITER, interval=150)
...
# Now, solve the TSP:
[nn_route, ip_route, sa_route, ga_route, nn_cost, ip_cost, sa_cost, ga_cost] = [[], [], [], [], -1, -1, -1,-1]
...
if (solveGA):
	# Solve the TSP using simulated annealing
	[ga_route, ga_cost] = solve_tsp_ga(objectiveType, distance_df, time_df,node)
...
if (turnByTurn):
	...
	# d) genetic algorithm:
		...
		folium.PolyLine(myShapepoints, color="black", weight=4.5, opacity=0.5).add_to(map_osm)
...
if (solveGA):
	...
	print "GA 'cost':"
	print ga_cost
# 	[ga_route, ga_cost, MEAN, BEST, RouteBest]
